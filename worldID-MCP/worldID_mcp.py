...
mcp = FastMCP("World-ID-Docs-MCP-Server")

@mcp.tool()
def world_id_query_tool(query: str) -> str:
    """
    Query the World ID documentation using a retriever.
...
@mcp.resource("docs://world-id/full")
def get_all_world_id_docs() -> str:
    """
    Returns the entire World ID docs text from world_id_full.txt
    (Essentially ~300k tokens of raw doc content).
    """
    doc_path = os.path.join(BASE_PATH, "llms_full.txt")
    if not os.path.exists(doc_path):
        return "llms_full.txt was not found. Did you run worldID_build_docs.py?"
    try:
